...
def test_mobile_calendar_cards_use_compact_poster_and_spacing() -> None:
    assert "@media (max-width: 720px)" in NAVIGATION_CSS
    assert ".calendar-page .calendar-entry {" in NAVIGATION_CSS
    assert "grid-template-columns: 68px minmax(0, 1fr);" in NAVIGATION_CSS
    assert ".calendar-page .calendar-entry-poster .poster," in NAVIGATION_CSS
    assert "width: 68px;" in NAVIGATION_CSS
    assert "height: 102px;" in NAVIGATION_CSS
    assert ".calendar-page .calendar-entry-copy {" in NAVIGATION_CSS
