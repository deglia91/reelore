...
class StubStore:
    def __init__(
        ...
        *,
        future_only: bool,
        status: LibraryStatus = LibraryStatus.COMPLETED,
    ) -> None:
    ...
    def get_personal_state(self, media_id: str) -> PersonalMediaState | None:
        return PersonalMediaState(media_id, self.status, completion_count=1)
...
def test_up_to_date_series_with_available_unseen_episode_returns_in_progress() -> None:
    updater = StubStatusUpdater()
    store = StubStore(future_only=False, status=LibraryStatus.UP_TO_DATE)
    service = TVStatusReconciliationService(store, updater)
...
def test_up_to_date_series_with_only_future_unseen_episode_stays_up_to_date() -> None:
    updater = StubStatusUpdater()
    store = StubStore(future_only=True, status=LibraryStatus.UP_TO_DATE)
    service = TVStatusReconciliationService(store, updater)

    result = service.reconcile(date(2026, 8, 21))

    assert updater.changes == []
